...
if __name__ == '__main__':
    ...
    for i in range(len(batch_size_list)):
        ...
        #print(np.shape(total_train_loss))
        #print(total_train_loss)
        train_loss_results.append(np.sum(total_train_loss, axis=1)/num_examples)
        test_loss_results.append(np.sum(total_val_loss, axis=1)/num_test_examples)

    #print(np.shape(train_loss_results))
    #print(train_loss_results)

    with open('accuracy_batch_size.csv', 'w', newline='') as f:
        write = csv.writer(f)
        write.writerows(test_acc_results)

    with open('train_loss_batch_size.csv', 'w', newline='') as f:
        write = csv.writer(f)
        write.writerows(train_loss_results)

    with open('test_loss_batch_size.csv', 'w', newline='') as f:
        write = csv.writer(f)
        write.writerows(test_loss_results)
